;(function($, window, document, undefined) {
	...
	$doc.ready(function() {
		...
		if ($win.width() < 1025) {
			$('.c-article-images-container-inner, .c-article-contents-container').bxSlider({
				mode: 'fade',
				adaptiveHeight: true,
				controls: false,
		...
		// Accordion
		$('.c-accordion-head').on('click', function(event) {
		...
		// Vimeo Player
		$('.c-video .btn-play').on('click', function(event) {
			$(this).closest('.c-video-inner').fadeOut(200);

			var $src = $(this).parents('.c-video').find('iframe').attr('src');

			$(this).parents('.c-video').find('iframe').attr('src', $src + '&autoplay=1');
			
			event.preventDefault();
		});

		// Popup Gallery
		$(".c-popup-gallery").fancybox();
		...
		$win.load(function() {
			...
			$('.c-slider-accordion').each(function() {
			var $this = $(this);
			var carousel = $this.find('.c-slides');
			var carouselPaging = $this.find('.c-slider-paging');
			var carouselSlideNext = $this.find('.c-slider-actions .c-slider-next');
